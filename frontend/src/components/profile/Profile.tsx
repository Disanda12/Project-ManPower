...
import { 
  User, Mail, Phone, MapPin, Camera, Save, ShieldCheck, 
  Loader2, Edit3, X, Calendar
} from 'lucide-react';
...
const ProfilePage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#fcfcfd] py-8 px-6 md:px-12">
            ...
            <div className="max-w-7xl mx-auto pt-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6 border-b border-gray-100 pb-8">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight">Profile Settings</h1>
                        <p className="text-gray-500 text-lg mt-2 font-medium">Update your account information and public profile.</p>
                    </div>
                    <button 
                    ...
                    >
                        {isEditing ? <><X size={22} /> Cancel</> : <><Edit3 size={22} /> Edit Profile</>}
                    </button>
                </div>
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-white border border-gray-100 rounded-[2.5rem] p-10 shadow-sm flex flex-col items-center sticky top-10">
                            <div className="relative group">
                                ...
                                <div className={`w-48 h-48 rounded-[3rem] overflow-hidden border-8 transition-all duration-500 ${isEditing ? 'border-blue-50 rotate-3 scale-105 shadow-2xl' : 'border-gray-50'}`}>
                                ...
                                {isEditing && (
                                    ...
                                    >
                                        <Camera size={32} />
                                        <span className="text-xs font-bold mt-2 uppercase tracking-widest">Change</span>
                                    </button>
                                )}
                            </div>
                            
                            <h2 className="mt-8 text-3xl font-bold text-gray-900 text-center">{profile.first_name} {profile.last_name}</h2>
                            <p className="text-gray-400 font-medium mt-1 uppercase tracking-tighter">Customer Account</p>
                            ...
                            <div className="w-full mt-10 pt-8 border-t border-gray-50 space-y-6">
                                <div className="flex items-center gap-4 text-gray-600">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-md font-bold text-gray-800">January 2026</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 text-gray-600">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-md font-bold text-gray-800">Verified Identity</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            <form onSubmit={handleSave} className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <CustomInput 
                                        label="First Name" icon={<User size={18}/>} value={profile.first_name} 
                                        isEditing={isEditing} onChange={(val) => setProfile({...profile, first_name: val})} 
                                    />
                                    <CustomInput 
                                        label="Last Name" icon={<User size={18}/>} value={profile.last_name} 
                                        isEditing={isEditing} onChange={(val) => setProfile({...profile, last_name: val})} 
                                    />
                                    <div className="space-y-3">
                                        <label className="text-sm font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 ml-1">
                                            <Mail size={16}/> Email Address
                                        </label>
                                        <input disabled value={profile.email} className="w-full p-5 rounded-2xl bg-gray-50 border border-transparent text-gray-400 cursor-not-allowed font-bold text-lg" />
                                    </div>
                                    <CustomInput 
                                        label="Phone Number" icon={<Phone size={18}/>} value={profile.phone} 
                                        isEditing={isEditing} onChange={(val) => setProfile({...profile, phone: val})} 
                                    />
                                    <div className="space-y-3 md:col-span-2">
                                        ...
                                        <textarea 
                                            ...
                                            className={`w-full p-5 rounded-2xl border-2 transition-all outline-none resize-none font-bold text-lg ${
                                                isEditing ? 'border-blue-50 focus:border-blue-500 bg-white' : 'border-transparent bg-gray-50 text-gray-600'
                                            }`}
                                        />
                                    </div>
                                </div>
